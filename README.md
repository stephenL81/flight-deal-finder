...
### main.py
Orchestrates all classes to execute the flight deal search workflow:
...
### flight_search.py
Handles all communication with the Duffel Flight API:
...
### data_manager.py
Manages Google Sheets data via the Sheety API:
...
### flight_data.py
Contains the core logic for finding and filtering flight deals:
...
Sends SMS notifications for flight deals via Twilio:
...
Securely stores all API credentials:
